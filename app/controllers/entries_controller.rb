class EntriesController < ApplicationController
  ...
  def index
    ...
    @entries.each do |entry|
      ...
      finalfour = Array.new(4)
      #entry.picks.find_all_by_round(6).each { |t| finalfour.each { |f| if t.samePick?(f) then finalfour.unshift(t) && finalfour.pop end } }
      #entry.picks.find_all_by_round(5).each { |t| finalfour.each { |f| if t.samePick?(f) then finalfour.unshift(t) && finalfour.pop end } }
      #entry.picks.find_all_by_round(4).each { |t| finalfour.each { |f| if t.samePick?(f) then finalfour.unshift(t) && finalfour.pop end } }
